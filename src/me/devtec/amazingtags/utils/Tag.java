...
public class Tag {
	 ...
	 */
	public String getTagPermission() { //Get special or default tag permission
	...
	public boolean canSee(Player player) {
		if(!isEnabled()) { //If tag is enabled in Tags.yml
			return false;
		}
		...
		return hasPermission(player); //If permission... :D
	}
	/** If tag is enabled
	 * @param tag - The name used in the file to access the tag data
	...
	public  boolean hasPermission(Player player) {
		if(getTagPermission()!=null && !getTagPermission().isEmpty()) {
			return player.hasPermission(getTagPermission());
		}
	 ...
	 */
	private ItemMaker getTagItemFromConfig() {
	 ...
	 */
	public  ItemStack getTagItem(Player player) {
		// Getting item
		ItemMaker item = getTagItemFromConfig();
		
		// Loading placeholders in item lore and name
		Placeholders placeholders = new Placeholders();
		placeholders.addPlayer("player", player)
			.replace("tag", getTagFormat()!=null?getTagFormat():"" ).replace("tag_format", getTagFormat()!=null?getTagFormat():"" )
			.replace("tag_name", getTagName()).replace("name", getTagName())
			.replace("info", getTagInfo()).replace("tag_info", getTagInfo())
			.replace("status", getStatus(player)).replace("tag_status", getStatus(player))
			;
	...
	public static ItemStack getPreviewItem(Player player) {
		...
		Tag tag = new Tag(API.getSelectedTag(player));
		Placeholders placeholders = new Placeholders();
		placeholders.addPlayer("player", player)
			.replace("tag", tag.getTagFormat()!=null?tag.getTagFormat():"" ).replace("tag_format", tag.getTagFormat()!=null?tag.getTagFormat():"" )
			.replace("tag_name", tag.getTagName()).replace("name", tag.getTagName())
			.replace("info", tag.getTagInfo()).replace("tag_info", tag.getTagInfo())
			.replace("status", tag.getStatus(player)).replace("tag_status", tag.getStatus(player))
			;
